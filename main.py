...
def summarize_news(news_list):
    ...
    # システム指示：思考プロセスや自己チェックを一切排除し、日本語の要約結果のみを出力させる
    system_instr = (
        "あなたは日本のAI技術専門のジャーナリストです。ニュースの内容を分析し、"
        "日本の読者が手短に理解しやすいよう、簡潔な日本語で要約・出力してください。"
        "【重要】要約以外のメタ情報（「はい、理解しました」「チェックリスト」「思考プロセス」など）は一切出力しないでください。"
        "出力は、指定された要約の本文のみとしてください。"
    )
    ...
    # プロンプト：出力形式を固定し、余計な解説を禁止する
    prompt = f"""以下のニュースリストから、重要なAI関連ニュースを3〜5個ピックアップし、以下の形式で日本語のみで出力してください。
挨拶や分析プロセス、自己評価などの「要約以外の文章」は一切含めないでください。
